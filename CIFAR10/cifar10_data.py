...
import tarfile
import cifar10_dataset_processor as processor
...
def distorted_inputs(data_dir):
  ...
  dataset = processor.distorted_inputs(data_dir=data_dir)
  return dataset
...
def distorted_test_inputs(data_dir):
    ...
    if not data_dir:
        raise ValueError('Please supply a data_dir')
    data_dir = os.path.join(data_dir, 'cifar-10-batches-bin')
    dataset = processor.distorted_test_inputs(data_dir=data_dir)
    return dataset
