...
def inference(images):
    ...
    with tf.variable_scope('layer1'):
        model = tf.layers.conv2d(inputs=images, filters=32, kernel_size=[3, 3], strides=(1, 1), padding='same', activation=tf.nn.relu, kernel_regularizer=tf.contrib.layers.l2_regularizer(0.1), name='conv')
        model = tf.layers.max_pooling2d(model, pool_size=[2, 2], strides=[2, 2], padding='same', name='pool')
